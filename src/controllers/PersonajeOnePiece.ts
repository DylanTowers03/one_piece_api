...
import {PersonajeOnePiece,IPersonajeOnePiece} from '../models/Personaje.model';

export default class PersonajeController {

    public async getAllPersonajes(req: Request, res: Response){
        try {
            const personajes: IPersonajeOnePiece[] = await PersonajeOnePiece.findAll();
    ...
    public async getPersonajeById(req: Request, res: Response){
        const { id } = req.params;
        try {
            const personaje: IPersonajeOnePiece | null = await PersonajeOnePiece.findByPk(id);
            if (!personaje) {
    ...
    public async createPersonaje(req: Request, res: Response){
        const { nombre, nivel, fruta, rol, imagen } = req.body;
        try {
            const personajeCreado: IPersonajeOnePiece = await PersonajeOnePiece.create({ nombre, nivel, fruta, rol, imagen });
    ...
    public async updatePersonaje(req: Request, res: Response){
        ...
        try {
            ...
            await PersonajeOnePiece.update({ nombre, nivel, fruta, rol, imagen }, { where: { id } });
